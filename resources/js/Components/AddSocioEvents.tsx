...
export default () => {
    ...
    const OnClickHandleSubmitData = (data: typeof defaultValues) => {
        router.post('events', {...data, date: date?.toDateString(), image: filesContent[0]}, {
            forceFormData: true,
            onSuccess: (data) => {
                toast.success('New Event Created', {
                    position: 'top-right'
                })
                getSocioEvents.push(data.props?.socioEvents as ISocioCulturalEvents)
                atomStore.set(socioEventsAtom, getSocioEvents)
            }
        })
    }
        ...
        <Dialog>
            ...
            <DialogPortal>
                <DialogContent>
                    ...
                    <form onSubmit={handleSubmit(OnClickHandleSubmitData)} className="space-y-5">
                        <div>
                        ...
                        <div>
                            ...
                            <Input type="text" placeholder={filesContent.length == 1 ? filesContent[0]?.name : 'Click to choose image'} readOnly onClick={openFilePicker} id="fileUpload" />
                        </div>
